# Prepravljno u odnsu na:
# https://gitlab.com/soilspec4gg/ossl_modeling/-/blob/master/api/server/R-code/functions/ml.R


# Load models from yml config file data_yml$default$models
ml.loadModels <- function(modelsConf) {
  models <- lapply(modelsConf, FUN = function(modelConf) list(model=readRDS(file = modelConf$rds), 
                                                              description = as.character(modelConf$description), 
                                                              evaluation = as.list(modelConf$evaluation))
  )
  names(models) <- names(modelsConf)
  return(models)
}

# Prepare csv file data - spectra for prediction
ml.predict.preprocessing <- function(newdata, spec.prefix = '', no_rows = no_rows, rm.fcol = TRUE) {
  
  spectra.col <- colnames(newdata)
  spectra.col.idx <- grep(spec.prefix, spectra.col)
  spectra.data <- newdata[spectra.col[spectra.col.idx]]
  
  colnames(spectra.data) <- gsub(spec.prefix, '', spectra.col[spectra.col.idx])
  spectra.data %<>% as.data.frame() 
  if(rm.fcol == TRUE){
    spectra.data %<>% dplyr::select(-1)
  } 
  spectra.data <- spectra.data[1:no_rows, ]
  return(spectra.data)
}

# ML prediction
ml.predict <- function(models, modelId, newData) {
  
  model <-  models[[modelId]]$model
  
  if ('plrs' %in% modelId) {
    
    ncomp.sel = as.data.frame(model$validation$PRESS) 
    colnames(ncomp.sel) = gsub("comps", "", names(ncomp.sel))
    ncomp = as.numeric(colnames(ncomp.sel[which.min(ncomp.sel[1,])])) 
    
    result <- as.data.frame(predict(model, newdata = newData, ncomp = ncomp))
    result[result < 0] = 0
    
    
  } else {
    result <- data.table(predict(model, newdata = newData, predict.all = TRUE))  
  }
  
  colnames(result) <- modelId
  result[[paste0(modelId,'.lower')]] = result[[modelId]] - models[[modelId]]$evaluation$rmse
  result[[paste0(modelId,'.upper')]] = result[[modelId]] + models[[modelId]]$evaluation$rmse
...
predict.ossl <- function(t.var, mir.raw, visnir.raw, lon, lat, hzn_depth=10, ossl.model, ossl.pca.mir, ossl.pca.visnir, spc.type="mir", subset.type="ossl", geo.type="na", n.spc=60, sd=TRUE, cog.dir="soilspectroscopy/layers1km/", ylim=NULL, dataset.code_ascii_c="KSSL.SSL"){ ## =c(0,100)
